...
export type FStartArgs = {
  ...
  remotesOrManifestUrl: string | undefined;
...
function applyDefaultArgs(args: FStartArgs) {
  ...
  args.remotesOrManifestUrl = args.remotesOrManifestUrl || defaultArgs.remotesOrManifestUrl;

  if (args.remotesOrManifestUrl && !fs.existsSync(args.remotesOrManifestUrl)) {
...
function exitWithUsage(defaultArgs: FStartArgs) {
  ...
  console.error('usage: nfstart ' + args);
  process.exit(1);
}
